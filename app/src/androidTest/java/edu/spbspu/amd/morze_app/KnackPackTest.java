...
public class KnackPackTest
{
  ...
  public void testRunPacking() throws Exception {
    ...
    for (iter = 0; (iter < MAX_ATTEMPTS) && (res == 1); iter++) {
      if ((iter & 127) == 127) {
        Log.d(m_log, "Iteration = " + String.valueOf(iter));
      }
    ...
    Log.d(m_log, "Num shapes = " + String.valueOf(numShapes));
  ...
  public void testRunUi() throws Exception
  {
    ...
    // wait for 3 sec
    Log.d(m_log, "TestRunUI. Wait 3 sec...");
    solo.sleep(WAIT_LARGE_TIME_MS);
    ...
    // System.out.printf("%s. AppIntro. Tested app is ready to press buttons = %d", m_log, isReady);
    // System.out.println(m_log + "AppIntro. Tested app is ready to press buttons = " + String.valueOf(isReady));
    if (isReady == 1)
      Log.d(m_log, "AppIntro. Tested app is ready to press buttons");
    else
      Log.d(m_log, "AppIntro. Tested app is NOT ready to press buttons: waiting...");
    while (isReady == 0)
    {
      isReady = appIntro.m_canPressButtons;
    }
    // System.out.printf("%s. AppIntro. Emulate press buttons in tested app", m_log);
    Log.d(m_log, "AppIntro. Emulate press button in tested app");

    rectf = appIntro.m_rectBtnStart;
    xClick = (int)(rectf.left + rectf.right) / 2;
    yClick = (int)(rectf.top + rectf.bottom) / 2;
    // System.out.printf("%s. AppIntro. Button coordinates to press = (%d, %d)", m_log, xClick, yClick);
    Log.d(m_log, "AppIntro. Click in point = " + String.valueOf(xClick) + ", " + String.valueOf(yClick));
